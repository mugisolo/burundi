...
export const subscribeToIncidents = (
    ...
    return onSnapshot(q, (snapshot) => {
        ...
        console.warn("Firestore Incident Subscription Error:", error.message);
...
export const subscribeToPresidential = (
    ...
    return onSnapshot(q, (snapshot) => {
        ...
        console.warn("Firestore Presidential Subscription Error:", error.message);
        if (onError) onError(error);
    });
};
...
export const subscribeToParliamentary = (
    onData: (data: ParliamentaryCandidate[]) => void,
    onError?: (error: any) => void
) => {
    if (!db) return () => {};
    ...
    return onSnapshot(collection(db, PARL_CANDIDATES_COL), (snapshot) => {
        ...
        console.warn("Firestore Parliamentary Subscription Error:", error.message);
        if (onError) onError(error);
    });
};
...
export const seedDatabase = async (
    ...
    try {
        ...
        if (e.code === 'permission-denied') {
            alert("Permission Denied: You do not have permission to write to this database. Please check your Firebase Security Rules in the console.");
        } else {
